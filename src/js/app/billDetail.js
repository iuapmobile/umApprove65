define(["zepto", "iscroll", "dialog", "components/ContactInfo", "components/Confirm"], function ($, isc, dialog, ContactInfo, Confirm) {
    //判断浏览器类型
        ...
        initTaskBillContent: function () {
            ...
            $(".order_document_ul li").on("click", function () {
                ...
                if (id == "goFlowChart") {
                    ...
                    window.location.href = "billContent.html?statuskey=" + paramData.statuskey + '&statuscode=' + paramData.statuscode + '&taskid=' + paramData.taskid;
                }
            });
            //点击查看附件列表页面，事件绑定
            $(".attachmentNumber").on("click", function () {
                window.location.href = "annex.html?filedowlnload=true";
            });
...
//	   		    	console.log("查询附件列表 返回JSON数据串为：" + JSON.stringify(result));
                    //{"code":"0","desc":"执行成功","jsonDatas":{"getMessageAttachmentList":[{"des":"获取附件列表成功","attachstructlist":[{"attachmentgrouplist":[{"filename":"NC5通讯录.txt","filetype":"other","filesize":"229B","fileid":"e9e5fe9d-8a33-4102-bcf4-cb90c424d6b9","downflag":"1"}],"attachmentgroupname":"附件"}],"flag":"0","count":"1"}]}}
                    //显示文件数目
        ...
        initPersonList: function (type) {
            var _person = "",
                personName = "";
            _bill_detail.loadPersonList(type, 2000, "", function(list) {
                        ...
                        if (checkResult.jsonDatas.getPermByUserCode == "0") {
                            ...
                            $.ajax({
                                ...
                                success: function (result) {
                                    ...
                                    if (result.code == "1") {
                                    	if(result.desc == ""){
                                    		dialog.log(eval("billDetailLang." + paramUser.lang + ".pageMessage.noFeeInfo") + result.desc );//"移动审批已欠费，请联系管理员"
                                    	}else{
                                    		dialog.log(eval("billDetailLang." + paramUser.lang + ".pageMessage.noFeeInfo") + "("+result.desc+")" );//"移动审批已欠费，请联系管理员"
                                    	}
                                    	$("#loding").hide();//隐藏加载中图标
...
// <<<
         ...
         * @param {object} budgetCtrl 预算控制参数，包含已处理的预算控制数组和待处理的预算控制
         * @param {object} paramBody 上一次使用的参数，在此基础上增加/更新预算控制即可
         * @returns {object} 审批参数
         */
         ...
         * @param {object} [paramBody] paramBody 上一次使用的参数，在此基础上增加/更新预算控制即可
         */
...
//	   		    	console.log(JSON.stringify(result));
                    ...
                    if (code != "0") {
                        ...
                        /*if(psnDetail.length > 0) {
                            ...
                            if (flag != "0") {
                                ...
                                $("#personInfo").html("");
                                $("#link").html("");
                                var personName = result.jsonDatas.getPsnDetail[0].pname,
                                    personSection = result.jsonDatas.getPsnDetail[0].pdes;
                                ...
                                var link = $('<div class="link" id="link">' +
                                    '<ul class="link_ul">' +
                                    '<li class="name" style="width:20%;">姓名</li>' +//人员信息
                                    '<li class="number" style="width:80%;"><span>' + personName + '</span></li>' +
                                    '</ul>' +
                                    '<ul class="link_ul">' +
                                    '<li class="name" style="width:20%;">部门</li>' +//人员信息
                                    '<li class="number" style="width:80%;"><span>' + personSection + '</span></li>' +
                                    '</ul>'
                                );
                                ...
                                for (var i = 0; i < contactinfolist.length; i++) {
                                    ...
                                    if (propvalue.indexOf("@") >= 0) {
                                        ...
                                        if(!isWeiXin()) {
                                            propnameShow = '<li class="number" style="width:80%;"><span><a href="tel:' + contactinfolist[i].propvalue + '">' + contactinfolist[i].propvalue + '</a></span></li>';
                                        } else {
                                    ...
                                    infoList += '<ul class="link_ul">' +
                                        '<li class="name" style="width:20%;">' + contactinfolist[i].propname + '</li>' + propnameShow +
                                        '</ul>';
                                }
                                //批接信息
        ...
        initIScroll: function () {
            ...
            $('#loding').show();
            setTimeout(function() {
                if (typeof(_iscroll) != "undefined" && _iscroll.destroy) {
                    _iscroll.destroy();
                }
