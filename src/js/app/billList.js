define(["zepto", "iscroll", "dialog", 'app/bpm'], function ($, isc, dialog, iUAPBPM) {
    //配置参数
...
// vvv6
        ...
        getBillList: function (_pagesCode, _buttonsCode, startline) {
            ...
            if (!startline) {
				...
				var BPM_currentBillSum = $('#' + pagesCode[pageIndex]).find('.bill[data-billsrc="bpmtaskcenter"]').length;//获取页面当前bpm单据数目
                ...
                NC_startline = NC_currentBillSum - BPM_currentBillSum + 1;
                OA_startline = OA_currentBillSum - BPM_currentBillSum + 1;
...
// vvv7
        ...
        get_NC_TaskListSuccessFunction: function (result) {
            ...
            if (result.code == "0") {
				...
				iUAPBPM.getBillList(result,arguments[1],function(newresult){
					var array = newresult.jsonDatas.getTaskList || newresult.jsonDatas.getTaskList65,
					...
					//billsArray = newresult.jsonDatas.getTaskList[0].taskstructlist;
        ...
        get_OA_TaskListSuccessFunction: function (result) {
            ...
            if (result.code == "0") {
				...
				result = this.get_OA_TaskListSuccessFunction_Ext(result)
		...
		get_OA_TaskListSuccessFunction_Ext: function(result){
			if(__BPMTaskCenterBillList.statusCode!=200){
				return result;
			}
			...
			var bpm_billsArray = [];
			for(var i=0,len=__BPMTaskCenterBillList.data.length;i<len;i++){
				var bpm_bill = {};			
				var item = __BPMTaskCenterBillList.data[i];
            ...
            for(var i=0, len=listArray.length; i<len; i++) {
                if(listArray[i]['data-system'] == 'OA') { // OA单据
                    oneBill += '<tr><td></td><td></td></tr>'
                        + '<tr class="bill"  id="billRowId_' + currentCountId + '"  data-billsrc="'+listArray[i].billsrc+'" data-system="OA" data-taskid="' + listArray[i].taskid + '">'
					...
					var _str_data_billsrc = "";
					if(listArray[i].billsrc == "bpmtaskcenter"){
						_str_data_billsrc = ' data-billsrc="' + listArray[i].billsrc + '"';
					}else{
						_str_data_billsrc = "";
					}
                    oneBill += '<tr><td></td><td></td></tr>'
                        + '<tr class="bill" id="billRowId_' + currentCountId + '"' + _str_data_billsrc + ' data-system="NC" data-taskid="' + listArray[i].taskid + '">'
                        + '<td width="5%" align="right" valign="top">' + pointFlagHtml + '</td>'
                        + '<td>'
                        + '<span class="nc_name" nameStr="'+listArray[i].title+"，提交日期为"+listArray[i].date+'"' + _str_data_billsrc + ' data-system="NC" data-taskid="' + listArray[i].taskid + '">' + _billList.substrLength(listArray[i].title, fontNumEach) + '<!--字数进行限制超过28个字用...显示--><span class="nc_color nc_margin">' + listArray[i].date.substr(5, 11) + '</span></span>'
                        + '</td>'
                        + '</tr>'
                        + '<tr>'
                        + '<td></td>'
...
// vvv11 ;
        ...
        billClickEvent: function () {
            $('#scroller').unbind().on('click', '.nc_name', function (event) {
				...
				var dataBillsrc = $(this).attr('data-billsrc');//单据来源系统，"NC"/"OA"
                ...
                if (dataSystem == "NC") {//NC表单，跳到NC表单详情
					var hrefbpm = "";
					if(dataBillsrc == "bpmtaskcenter"){
						 hrefbpm = "bpm";
						 alert("即将跳转到bpmtaskcenter单据详情...")
					}
					window.location.href = hrefbpm + 'billDetail.html?statuskey=' + pagesCode[pageIndex] + '&statuscode=' + buttonsCode[headerBtnIndex] + '&taskid=' + taskid+'&imTitle='+imTitle;
				} else {//OA表单，跳到OA表单详情
