define(["zepto", "iscroll", "dialog"], function ($, isc, dialog) {
    ...
    $('body').on('goImEvent','.webview_GoIm_img',function(){
        ...
        var isOA = true ;
        uapmobile.goIm(usercode , taskid,appkey,domain,goIm_psid , isOA );
    })
                ...
                success: function (rtnData) {
                    ...
                    if (rtnData.flag == "0") {
                        ...
                        if (rtnData.data.length > 0) {
                            ...
                            continueFlag = false;
                            _bill_detail.show_nodata(eval("billDetailLang." + get_lang() + ".pageMessage.neverEnterMobileApprove"));
                            return false;
                        }
                    } else {
                        continueFlag = false;
        ...
        getCommonData: function (urlparam) {
            ...
            try {
                ...
                if (urlparam == "" || urlparam == undefined) {
                    ...
                    //判断下是否调用后台解析登陆
                    if (window.sessionStorage.urlparam == urlparam) {
                        //直接加载数据
                        paramUser = JSON.parse(window.sessionStorage.paramUser);

// 判断 是否存在流程图 ;
// 必须等到 paramUser 存在在调用 ;
_bill_detail.initTaskFlowChart();// 是否有流程图 ;
        ...
        initLocalData: function (result) {
            ...
            if (!user.oatoken) { // 没有通过getUserInfoFromCloud获取到oatoken
                user.oatoken = result.data.foot.oatoken || result.data.foot.nctoken || result.data.foot.token;
            }
            if (!user.groupid) { // 没有通过getUserInfoFromCloud获取到groupid
                user.groupid = result.data.foot.groupid || result.data.foot.pk_group;
            }
...
_bill_detail.initTaskFlowChart();// 是否有流程图 ;
        ...
        lightAppMultilingual: function () {
            ...
            for (var i in eval("billDetailLang." + lang + ".pageContent")) {
                //i即属性名字ok,close
//					console.log("key===" + i + ",value==" + eval("billDetailLang." + lang + ".pageContent." + i));
                $("." + i).html(eval("billDetailLang." + lang + ".pageContent." + i));
            }
            //再设置页面中的placeholder中的汉字
            for (var i in eval("billDetailLang." + lang + ".pagePlaceHoder")) {
                //i即属性名字ok,close
//					console.log("placeholder key===" + i + ",value==" + eval("billDetailLang." + lang + ".pagePlaceHoder." + i));
        ...
        initListHeight: function () {
            ...
            if (paramData.statuskey == "submit") { //我的发起
                loadNumber++;
                ...
                success: function (result) {
                    ...
                    try {
                        if (result.jsonDatas.getMainBody[0].contenttype) {//如果正文存在
                            ...
                            paramData.filets = mainBody.filets;//文件filets
                ...
                if (id == "zhenWen") {
                    if (paramData.mainbodyid == "") {
                        window.location.href = "OAbillContent.html?statuskey=" + paramData.statuskey + '&statuscode=' + paramData.statuscode + '&taskid=' + paramData.taskid
                            + '&mainbodyid=""&downflag=' + paramData.downflag + '&contenttype=' + paramData.contenttype
                            + '&fillename=' + paramData.fillename + '&filesize=' + paramData.filesize+'&filets='+paramData.filets;
                    }else {
                        window.location.href = "OAbillContent.html?statuskey=" + paramData.statuskey + '&statuscode=' + paramData.statuscode + '&taskid=' + paramData.taskid
                                    ...
                                    $(".task_other_list,.task_detail").not($(".border_lfrt")).on("click", function () {
                                        ...
                                        if (paramData.hint != "" && paramData.hint != undefined) {
                                            dialog.confirm(paramData.hint + "是否继续?", function () {
                                                //跳转到审批页面
                                                var gourl = "OAdoAction.html?statuskey=" + paramData.statuskey + '&statuscode=' + paramData.statuscode + '&taskid=' + paramData.taskid
                                                    + "&code=" + obj.attr("data-code") + "&codeName=" + $(this).text() + "&actflag=" + obj.attr("data-actflag")+"&fromOADetail=fromOADetail";
                                                    // 从审批中心进入 , 调回审批中心 ;
